...
public class Screen {
	...
	public boolean deleteShape(ShapeType shapeType, Point point, List<Integer> parameters){
		for(int i = 0; i < shapeList.size(); i++){
			...
			double xOfShape = shape.getOrigin().getXCoordinate();
			double yOfshape = shape.getOrigin().getYCoordinate();
			//check the shape to be removed
			if(shape.getShapeType().equals(shapeType) && x == xOfShape && y == yOfshape && shape.getParameters().equals(parameters) ){
